...
def sources_save(data,source_id=None):
    ...
    if merged['adapter'] not in {'rss','arxiv','huggingface','openreview','github_releases','legacy','pages'}:
...
def merge(target_id,record_ids,reason):
    ...
    target=store.get_record(target_id)
    records=[store.get_record(i) for i in set(record_ids) if i!=target_id]
    ...
    # Soft grouping retains each original identity and source; no records/evidence are destroyed.
    with get_db() as db:
        db.execute('INSERT INTO knowledge_editorial_actions(id,action,payload,reason,created_at) VALUES(?,?,?,?,?)',(action_id,'merge',store.encode(payload),reason,store.now()))
        for r in records:
            r['metadata']['merged_into']=target_id; r['metadata']['editorial_override']=True
            db.execute('UPDATE knowledge_records SET metadata=?,updated_at=? WHERE id=?',(store.encode(r['metadata']),store.now(),r['id']))
            db.execute('INSERT INTO knowledge_changes(record_id,action,snapshot,reason,changed_at) VALUES(?,?,?,?,?)',(r['id'],'grouped',store.encode(r),reason,store.now()))
...
def undo(action_id,reason):
    with get_db() as db:
        ...
        for original in payload['records']:
            current=db.execute('SELECT * FROM knowledge_records WHERE id=?',(original['id'],)).fetchone()
            record=store.row_record(current)
            if record['metadata'].get('merged_into')!=payload['target_id']: raise ValueError('Grouping changed; review before undo')
            # Retain corrections made after grouping, remove only this grouping marker.
            record['metadata'].pop('merged_into',None)
            db.execute('UPDATE knowledge_records SET metadata=?,updated_at=? WHERE id=?',(store.encode(record['metadata']),store.now(),record['id']))
            db.execute('INSERT INTO knowledge_changes(record_id,action,snapshot,reason,changed_at) VALUES(?,?,?,?,?)',(record['id'],'ungrouped',store.encode(record),reason,store.now()))
        db.execute("UPDATE knowledge_editorial_actions SET status='undone' WHERE id=?",(action_id,))
    return {'ok':True}
...
def resolve(conflict_id,choice,reason):
    ...
    record=store.get_record(row['record_id'],include_withdrawn=True)
    record['facts'][row['field']]=options[choice]
    record['metadata']['editorial_override']=True
    store.save_record(record,'Resolved evidence conflict: '+reason,record['id'])
    with get_db() as db:
        db.execute("UPDATE knowledge_conflicts SET status='resolved',resolution=?,resolved_at=? WHERE id=?",(reason,store.now(),conflict_id))
    return {'ok':True}


def review_queue(status='',query=''):
    with get_db() as db:
        rows=db.execute('SELECT * FROM knowledge_records WHERE title LIKE ? ORDER BY updated_at DESC LIMIT 200',('%'+query+'%',)).fetchall()
    ...
    if status: records=[r for r in records if r['status']==status or r['completeness']==status]
    return {'records':records,'conflicts':conflicts,'actions':actions,'feedback':feedback,'truncated':len(rows)==200}
