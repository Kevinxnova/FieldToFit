...
import { AliasNotice } from '../components/workspace/PublicMaintenance';
...
function Detail({ id, revision, onClose }: { id: string; revision: string; onClose: () => void }) {
  ...
  return <section className="platform-detail"><VisitReady ready={!!pub && !!obj && !result.loading && !result.error}/><button className="text-button" onClick={onClose}>← {pick("返回资源档案", "Back to dossiers")}</button><LoadState {...result} retry={result.reload} />{pub && obj && <>
...
export function ForAI() {
  ...
  const instructions = pick(
    `请将 FieldToFit 添加为只读远程 HTTP MCP 服务。地址：${endpoint}。配置示例：\n${config}\n先用 curated_lookup 按名称、别名、编号或关键词统一检索动态、持续关注和可公开原文；按结果中的 reading 读取完整整理内容，按材料 reading 使用 curated_material 继续读取原文，也可用 curated_bundle 打包。保留片段与全文的区别。后续用 curated_changes 的 scope=workspace 查询当前动态、持续关注及材料失效/恢复变化，保留该范围自己的续读游标；旧资料库使用 scope=legacy，游标不能混用。保留来源、版本、核验日期与未知项；编辑解读不等于原文，链接资料请按需继续读取。再结合我提供的需求继续工作。如果你无法添加 MCP，请指导我在客户端的 MCP/连接器设置中添加；不要声称已经连接。`,
  ...
  return <div className="platform-page"><VisitReady/><header className="platform-hero"><p className="platform-eyebrow">FOR YOUR AI · {pick('给 AI 用','SOURCE MATERIALS')}</p><h1>{pick('把 FieldToFit，\n接入你的 AI。','Connect FieldToFit.\nGive your AI the sources.')}</h1><p className="platform-lead">{pick('这里提供与 For you 同源的近期动态、持续关注资料、版本和出处。复制 MCP 接入说明给你的 AI，或在支持远程 MCP 的客户端中添加服务；接入后，让它按你的问题检索和读取。暂时无法接入，也可以下载下方资料直接交给 AI。','Access the same developments, profiles, versions and sources as For you. Copy the MCP setup instructions for your AI or add the service in a remote-MCP client, then ask it to read the materials relevant to your question. You can also download profiles below.')}</p><div className="platform-actions"><button className="button primary" onClick={()=>copy(instructions)}>{pick('复制 MCP 接入说明','Copy MCP setup instructions')}</button><a className="button" href="#ai-materials">{pick('浏览与下载资料','Browse and download')}</a></div><p className="muted">{pick('复制说明不会自动完成连接；不同客户端的配置格式可能不同。公开只读，无需注册。','Copying does not automatically connect your client; configuration formats vary. Public, read-only access needs no registration.')}</p>{copyFailed&&<textarea aria-label={pick('MCP 接入说明','MCP setup instructions')} readOnly rows={8} value={instructions}/>}</header>
    ...
    <UnifiedLookup key={'lookup-'+accessRevision}/><div id="ai-materials"><WatchAI key={accessRevision}/></div><DeveloperProjects key={accessRevision} ai/>
    <section className="platform-panel"><h2>{pick('接入后，可以这样问','After connecting, try this')}</h2><blockquote>{pick('从 FieldToFit 读取最近的 AI 动态，以及我关注的模型或项目资料。先列出发生了哪些变化和对应来源，再结合我接下来提供的任务继续分析；区分官方声明、编辑解读和没有核实的信息。','Read recent developments and the models or projects I follow from FieldToFit. List changes and sources, then analyze them against the task I provide. Distinguish official claims, editorial notes and unverified information.')}</blockquote><ul className="platform-reading-steps"><li><code>curated_lookup</code><p>{pick('统一发现动态、持续关注和可公开原文；返回命中原因、出处与继续读取方式。','Discover developments, profiles and permitted stored text together, with match reasons, sources and reading instructions.')}</p></li><li><code>curated_news</code><p>{pick('公司与产品动态、分点解读、关联资料和原始出处。','Company and product developments, editorial points, related materials and sources.')}</p></li><li><code>curated_watch</code><p>{pick('模型、工具、Agent、Skill、Harness 的介绍、版本表、解读和核验日期；支持关键词和类型筛选。','Model, tool, Agent, Skill and Harness profiles, version tables, notes and review dates; filter by keyword and type.')}</p></li></ul><p className="muted">{pick('每项材料会区分已存原文、部分片段和仅有链接；中文整理不冒充原文。每 1 天检查是维护周期，实际内容以资料中的核验日期为准。','Materials distinguish stored text, excerpts and link-only sources. Editorial notes are not source text. Daily checks are the maintenance cadence; refer to each record’s review date.')}</p></section>
  </div>;
}

export function AboutFieldToFit() {
  const { pick } = useWorkspace(); const health = useRemote<{version:string}>('/health');
  return <div className="platform-page platform-about"><VisitReady/>
