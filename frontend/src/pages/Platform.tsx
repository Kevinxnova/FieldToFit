...
export function ForYou() {
  ...
  return <div className="platform-page">
    ...
    <p className="reading-page-summary">{pick('本页包含', 'On this page:')} {news.data?.total ?? '—'} {pick('条动态与', 'developments and')} {result.data?.total ?? '—'} {pick(filtered ? '项匹配资源档案。资源筛选不影响近期动态。' : '项资源档案。先读近期变化，再查持续维护的资料。', filtered ? 'matching dossiers. Dossier filters do not filter the news.' : 'dossiers. Read recent changes, then explore the maintained materials.')}</p>
    <div className="reading-layout">
    <ReadingContents newsTotal={news.data?.total ?? null} news={(news.data?.items || []).map(n => ({id:newsAnchor(n.id),title:n.name}))} resources={(result.data?.items || []).filter(p => !p.unavailable).map(p => ({id:'resource-'+p.object.id,title:p.object.name}))} resourceTotal={result.error ? null : result.data?.total ?? null} loading={result.loading} filtered={filtered} />
    ...
    <section className="dossier-section"><div className="platform-section-heading"><h2 id="resource-dossiers" tabIndex={-1}>{pick("资源档案", "Resource dossiers")}</h2><span>{result.data?.total ?? '—'} {pick("项已审核对象", "reviewed objects")}</span></div>
