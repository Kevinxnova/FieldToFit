...
function Detail({ id, revision, onClose }: { id: string; revision: string; onClose: () => void }) {
  ...
  return <section className="platform-detail"><button className="text-button" onClick={onClose}>← {pick("返回资料流", "Back to the feed")}</button><LoadState {...result} retry={result.reload} />{pub && obj && <>
...
export function ForYou() {
  ...
  return <div className="platform-page">
    <header className="platform-hero"><p className="platform-eyebrow">FOR YOU · {pick("给你看", "CURATED READING")}</p><h1>{pick("看见重点，\n读到来处。", "Find what matters.\nRead the source.")}</h1><p className="platform-lead">{pick("值得关注的 AI 对象与变化，经过整理，保留依据。看懂之后，把原始材料交给你的 AI。", "Selected AI objects and developments, organized with evidence. Understand the essentials, then give the original material to your AI.")}</p><div className="platform-hero-meta"><span>{pick("每 1 天检查 · 少量精选", "Checked daily · Curated scope")}</span><Link to="/about">{pick("为什么做 FieldToFit", "Why FieldToFit")} ↗</Link></div></header>
    {params.has('legacy') && <p className="platform-notice">{pick("你从旧入口来到这里。旧领域与任务筛选已退出主导航，这里只展示经过审核的精选资料。", "You followed a legacy entry. This feed contains reviewed selections; the old domain and task filters are no longer primary navigation.")}</p>}
    <EditionOverview />
    <div className="platform-section-heading"><h2>{pick("精选资料", "Selected materials")}</h2><span>{result.data?.total ?? '—'} {pick("项已审核对象", "reviewed objects")}</span></div>
    <SourceContext /><form className="platform-search" onSubmit={submit}><input aria-label={pick("搜索精选对象", "Search selected objects")} placeholder={pick("搜索名称或关键词", "Search names or keywords")} value={query} onChange={e => setQuery(e.target.value)} /><button className="button" type="submit">{pick("搜索", "Search")}</button><details><summary>{pick("按类型筛选", "Filter by type")}</summary><select aria-label={pick("精选对象类型", "Selected object type")} value={params.get('type') || ''} onChange={e => setParams(p => { p.set('type', e.target.value); p.delete('offset'); p.delete('cursor'); return p; })}><option value="">{pick("全部类型", "All types")}</option>{Object.entries(typeNames).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></details></form>
    <section className="platform-panel platform-selection" aria-label={pick('选择资料包内容', 'Select package contents')}><div className="platform-section-heading"><h3>{pick('选择要交给 AI 的资料', 'Choose materials for your AI')}</h3><span>{chosen.length} / 10</span></div><p>{pick('在下方勾选对象，可跨搜索和分页选择。本次选择保留各自的发布修订，刷新页面后清空。', 'Select objects below, including across searches and pages. Each selection keeps its publication revision; reloading clears the selection.')}</p>{chosen.length > 0 && <><ul>{chosen.map(ref => <li key={ref.id}><span>{ref.name} · r{ref.revision}</span><button className="text-button" aria-label={pick('移除 ', 'Remove ') + ref.name} onClick={() => setChosen(previous => previous.filter(r => r.id !== ref.id))}>{pick('移除', 'Remove')}</button></li>)}</ul><button className="text-button" onClick={() => setChosen([])}>{pick('清空选择', 'Clear selection')}</button><MaterialPackage key={chosen.map(r => `${r.id}:${r.revision}`).join(',')} references={chosen} /></>}{chosen.length === 10 && <p role="status">{pick('已选满 10 项；移除后可加入其他对象。', '10 objects selected; remove one to add another.')}</p>}</section>
