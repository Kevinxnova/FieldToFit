...
更新日期：2026-09-11。**现行需求与状态唯一入口。** 当前应用版本 v1.0.0，既有 27 项资源档案。P3 已实现近期动态（速览/发布与更新）、资源档案、逐点 FieldToFit 解读、页内目录与 10 条首发动态（含 DeepSeek-V4.1-Flash）；增加 curated_news，精选 MCP 共 11 项。验证与发布记录见[本轮验收](../validation/2026-09-11-reading.md)。来源跟踪表、完整新闻原文采集及生产持续运营仍未完成。

[项目立意](goals.md) · [三页方案与文案](pages.md) · [资料与读取契约](data-contract.md) · [验收](acceptance.md) · [路线图](../../ROADMAP.md)

## 范围与状态规则

- 当前范围为 **19 个 REQ、76 个子 REQ**。使用 `REQ-` 前缀区别于历史编号；子项以 `.01` 等后缀标识。下面链接逐项给出用途、方案、验收、依赖和状态。
...
| [REQ-Y-02](requirements/for-you.md#req-y-02) | For you 动态与资源档案 | .01–.04 | 部分实现；范围与缺口见子项 | S2 |
...
4. 仓库检查器已同时校验 19 个父项、76 个子项及 42 个历史锚点，输出三类独立计数；见 REQ-X-01.04。
