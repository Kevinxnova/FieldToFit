# FieldToFit REQ list · 平台基线 P1

更新日期：2026-09-10。**这里是现行需求与状态的唯一入口。** 目标是 For you、For your AI 两个主页面及辅助“关于 FieldToFit”页面。当前代码仍为 **v1.0.0**；已进入 Unreleased 功能开发：精选基础、三个页面、概览期次归档、稳定分页、变化读取、多对象含原文资料包、精选采集源状态及来源/日期筛选可本地使用。公开修订历史也已接入。最新证据见[公开历史与网页补验](../validation/2026-09-10-platform-history.md)。

[项目立意](goals.md) · [三页方案与文案](pages.md) · [资料与读取契约](data-contract.md) · [验收](acceptance.md) · [路线图](../../ROADMAP.md)

## 范围与状态规则

- 当前范围为 **18 个 REQ、72 个子 REQ**。使用 `REQ-` 前缀区别于历史编号；子项以 `.01` 等后缀标识。下面链接逐项给出用途、方案、验收、依赖和状态。
- 全部属于本轮平台目标；按路线图分阶段完成。现有功能可复用，但没有一个新父 REQ 已完整通过新标准。各子项按本次实现更新；证据见[基础检查](../validation/2026-09-09-platform-foundation.md)及[期次与增量检查](../validation/2026-09-09-platform-updates.md)。
...
- 案例 pending；注册、登录、同步待开放；任务选型比较、场景和执行计划生成不在新主线。
- 旧 200 资源 / 100 论文 / 60 任务包 / 12 案例目标失效。先维护约 20 个精选对象，可更少；发布质量门槛优先。
...
| [REQ-Y-01](requirements/for-you.md#req-y-01) | For you 本期概览 | .01–.04 | 部分实现；范围与缺口见子项 | S2 |
| [REQ-Y-02](requirements/for-you.md#req-y-02) | For you 精选资料流 | .01–.04 | 部分实现；范围与缺口见子项 | S2 |
...
**下表只用于旧链接和版本记录的追溯，不是现行 42 项完成率。** 调整前的逐项状态与原始验收要求见[完整快照](../archive/2026-09/2026-09-08-before-platform-requirements.md)。历史版本说明继续描述当时交付，不改写成新目标的完成证据。
...
4. 仓库检查器已同时校验 18 个父项、72 个子项及 42 个历史锚点，输出三类独立计数；见 REQ-X-01.04。
