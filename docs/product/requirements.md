...
更新日期：2026-09-12。**现行需求与状态唯一入口。** 当前应用版本 v1.1.1。P3 + CW1 已实现 10 条近期动态、五类 27 个持续关注主体、结构化版本/技能表、分点解读、页内目录及同源 curated_watch；精选 MCP 共 12 项。原有数据库及兼容 API 保留，页面删除旧归档/资料库入口。两来源 2026 年模型图表、公司配色、直接名称标注与各家旗舰筛选已实现；保留双页导读；图表证据见[v1.0.3 验收](../validation/2026-09-11-v1.0.3.md)，前版卡片与说明调整见[v1.0.5 验收](../validation/2026-09-12-v1.0.5.md)。[持续关注历史验收](../validation/2026-09-11-continuous-watch.md)。v1.1.0 新增统一内容管理、每日审阅→草稿→双端预览→发布及原样迁移，见[本版验收](../validation/2026-09-12-v1.1.0.md)。来源跟踪表、完整原文采集、统一对象关系及持续运营仍未完成。
...
v1.1.1 统一版本记录、双语 README 和发布检查，见[验收](../validation/2026-09-12-v1.1.1.md)；不改变需求数量或既有功能完成状态。
