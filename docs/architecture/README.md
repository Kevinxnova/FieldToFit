...
beta.2 已加入显式事务与失败回滚，并有模拟远程传输检查；生产 Turso 公开读取已验；生产维护事务、超时恢复和完整备份恢复仍待实测。运行脚本有清单、临时目录和超时控制，没有完整 CPU/内存/网络隔离；通用运行沙箱不在新平台范围。原技术记录保留，新的部署边界见 [REQ-O-03](../../FieldToFit-PM.md#req-15)。
