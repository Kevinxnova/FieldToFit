...
| 路径 | 用途与处理 |
| --- | --- |
| `frontend/` | 现行页面、构建与 5 份网站图片；新增 /community，移除账户菜单但保留兼容路由 |
...
v1.1.0 新增六张内容管理表并原样迁入三个公开集合；保留旧数据库、定时配置、管理员凭证与兼容 API，没有批量删除 Python/前端代码。资料保留是否必要需由调用链和运行证据决定，不能只搜索旧名称。
