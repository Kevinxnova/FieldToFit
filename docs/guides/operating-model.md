...
2026-09-10。面向 REQ-F-04、REQ-F-05、REQ-AI-03、REQ-O-01～O-03。

本文件区分已实现的交接能力与仍需用户对齐的长期运行方式。当前没有创建本机定时任务，没有把个人 Codex 登录凭据部署到服务器，也没有完成公网部署。已恢复 Vercel 登录，按用户要求新建项目并更名为 fieldtofit。原有 Metis 数据库完成备份恢复检查，27 项审核内容已导入空精选表；fieldtofit.top 已关联，DNS 待更新。

## 平台分工

网站维护同一份已审核资料。For you 展示中文介绍、事实、关注依据、限制和变化；For your AI 的索引、原文、资料包及 MCP 引用同一发布修订。个人 AI 自行理解和使用材料，网站不生成用户任务方案。
...
| 首批维护名单 | 从真实材料选择可维护对象，约 30 为参考；不平均凑类型，不把弃用项目宣传为新推荐 | 每项有明确来源、关注理由和可读主要原文 |
...
公开精选 MCP 使用 `/api/mcp/curated`，只提供精选读取能力；原 `/api/mcp` 保留旧工具兼容。域名计划为 `https://fieldtofit.top`，目标地址不等于已经部署成功。
