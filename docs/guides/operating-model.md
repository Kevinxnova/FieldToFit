...
旧对象资料可通过审核数据库发布；当前近期动态、持续关注和模型图表使用版本化文件，修改后需核验并部署。每个有实际改动的交付提交（含内容/文档）都递增 z；y/x 升级先对齐确认。软件更新则走：隔离开发 → 测试和构建 → 预发布及数据恢复验证 → 更新版本记录 → 正式部署 → 健康与 MCP 检查。回滚同时考虑代码和相容数据备份。
...
模型图表的每日可用性检查随现有 cron 执行，运行类型为 model_landscape_daily。评分/价格与源日期仍需人工核对，检查不会自动更新快照；详见[图表维护](../product/model-landscape.md)。
