...
| 页面 | 当前可以做什么 | 仍未完成 |
| --- | --- | --- |
| `/for-you` | 人工审核的本期/往期概览、已审资料流、名称/类型/来源/发布日期搜索、固定修订详情、事实/来源/原文、复制资料清单、手选集合及含原文 JSON/Markdown 预览下载 | 资料流人工排序、新详情本地收藏 |
...
| `/admin` | 在对象编辑中整理精选介绍、别名、角色、来源引文和材料，预览准入缺口并审核发布；新增概览期次管理 | 统一精选问题队列、修改者身份审计 |
