...
(async()=>{
 ...
 try{
  const base=process.env.READING_BASE_URL||'http://127.0.0.1:18004',out=process.env.READING_OUTPUT||'/tmp/fieldtofit-v102';fs.mkdirSync(out,{recursive:true});
  ...
  assert.equal(await page.locator('.chart-company-legend button').count(),13);
  const colors=[];
  for(const source of snapshot.sources){
   ...
   await page.locator('.chart-company-legend button').filter({hasText:'DeepSeek'}).click();assert.equal(await plot.locator('.chart-point').count(),source.points.filter(p=>p.organization==='DeepSeek').length);assert.deepEqual([await dot.getAttribute('cx'),await dot.getAttribute('cy')],position);
   ...
   await page.getByRole('button',{name:'重置筛选',exact:true}).click();await plot.locator('.chart-point').first().waitFor();assert.equal(await plot.locator('.chart-point').count(),source.points.length);
  ...
  const health=await(await context.request.get(base+'/api/health')).json();assert.equal(health.version,'v1.0.2');await page.goto(base+'/about');assert.match(await page.locator('.platform-eyebrow').first().textContent(),/v1\.0\.2/);
  ...
  assert.deepEqual(errors,[]);fs.writeFileSync(out+'/result.json',JSON.stringify({passed:true,base,version:health.version,checks:['two sources / Epoch URL fallback','190 exact source model labels / no overlaps','12 consistent company colors','fixed coordinates during filters / search / empty / reset','keyboard tabs / URL reload / point evidence','coverage lists / genuine dates','zoom / Escape focus / mobile pan / no page overflow','about and health version','chart failure preserves news and watch'],coverage:snapshot.sources.map(s=>({source:s.id,...s.coverage}))},null,2));console.log('v1.0.2 browser acceptance passed');
