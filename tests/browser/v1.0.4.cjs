...
(async()=>{
 ...
 await toggle.click();assert(await drawer.isVisible());assert.equal(await p.evaluate(()=>getComputedStyle(document.body).overflow),'hidden');await p.keyboard.press('Escape');await p.waitForFunction(()=>!document.querySelector('#reading-toc-drawer').open);assert.equal(await toggle.evaluate(n=>n===document.activeElement),true);
 await toggle.click();await p.screenshot({path:out+'/directory-mobile.png'});await drawer.getByRole('link',{name:'开发者投稿项目',exact:true}).click();await p.waitForFunction(()=>document.activeElement?.id==='developer-projects');assert(!(await drawer.isVisible()));assert.notEqual(await p.evaluate(()=>getComputedStyle(document.body).overflow),'hidden');assert(p.url().includes('chart=arena'));
